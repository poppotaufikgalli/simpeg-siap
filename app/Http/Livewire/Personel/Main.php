...
class Main extends Component
{
    ...
    {
        $retData = MasterPersonel::where('id_jenis_personel', '=', $this->id_jenis_personel)->where(function($query){
            if($this->id_opd != ""){
                if($this->id_sub_opd != ""){
            ...
            if($this->keselon != ""){
                $query->where('keselon', '=', $this->keselon);
            }
        })->Paginate(20)->withQueryString();

        $this->resetPage();

        return view('livewire.personel.main', [
            'pegawai' => $retData,
