<div>
	...
	<div class="d-flex justify-content-end mb-3">
		<a href="{{route('config.create',['page' => $page, 'ref' => 0])}}" class="btn btn-sm btn-primary">Tambah Menu</a>
	</div>
				...
				@if(isset($data))
					...
					@foreach($data['main'] as $key => $value)
						<tr>
							...
							<td class="ms-{{$lvl}}">{{ $value->judul }}</td>
						...
						@php($id_ref = $value->id)
						@include("admin.config.partials.submenu-table", ['sub' => $data['sub'], 'id_ref' => $id_ref])
					@endforeach
				@endif
			</tbody>
		</table>
