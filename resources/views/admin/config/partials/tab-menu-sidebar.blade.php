<div>
	@php($page="menuSidebar")
	<div class="d-flex justify-content-end mb-3">
		<a href="{{route('config.create',['page' => $page, 'ref' => 0])}}" class="btn btn-sm btn-primary">Tambah Sidebar</a>
	</div>
			...
			<tbody>
				@if(isset($menuSidebar))
					...
					@foreach($menuSidebar['main'] as $key => $value)
						...
						@php($id_ref = $value->id)
						@include("admin.config.partials.submenu-table", ['sub' => $menuSidebar['sub'], 'id_ref' => $id_ref])
					@endforeach
				@endif
			</tbody>
		</table>
